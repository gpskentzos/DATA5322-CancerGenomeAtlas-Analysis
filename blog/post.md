...
When four independent algorithms, using different mathematics and different assumptions, independently recover the same underlying structure - one that matches what clinical research found through a completely different route - that convergence is not a coincidence. It is evidence that the structure is genuinely there, encoded deeply in the biology of how these tumors express their genes.
The original question was whether an algorithm could rediscover what oncologists took decades to find. The answer is yes - and it took us five notebooks, 529 patients, and 17,000 genes to show it. Turns out, the biology doesn’t lie - not even to a machine that doesn’t know what biology is. 
